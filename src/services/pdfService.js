...
export const generateReportePDF = async (reporte, carroceria = {}, usuario = {}, prioridad = {}, severidad = {}, imagenAnalizada = {}) => {
  try {
    ...
    // Función simple para convertir colores problemáticos
    const safeColor = (color) => {
      if (!color) return '#808080';
      if (color.startsWith('#')) return color;
      if (color.startsWith('oklch') || color.startsWith('hwb') || color.startsWith('lab') || color.startsWith('lch')) {
        console.log('Color moderno detectado, usando fallback:', color);
        return '#808080';
      }
      return '#808080'; // Fallback seguro
    };
    ...
    pdfContainer.innerHTML = `
      ...
      ${imagenAnalizada?.imperfecciones ? `
        <div style="border: 2px solid #f59e0b; border-radius: 8px; padding: 20px; margin-bottom: 30px;">
          ...
          <div style="background-color: #fffbeb; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
              <div>
                ...
                <p style="margin: 8px 0;"><strong>Total de imperfecciones:</strong> ${imagenAnalizada.imperfecciones.length}</p>
                <p style="margin: 8px 0;"><strong>Densidad:</strong> ${imagenAnalizada.imperfecciones.length > 20 ? 'Alta' : imagenAnalizada.imperfecciones.length > 10 ? 'Media' : 'Baja'}</p>
              ...
              <div>
                ...
                <p style="margin: 8px 0;"><strong>Rango X:</strong> ${Math.min(...imagenAnalizada.imperfecciones.map(i => i.x))} - ${Math.max(...imagenAnalizada.imperfecciones.map(i => i.x))}</p>
                <p style="margin: 8px 0;"><strong>Rango Y:</strong> ${Math.min(...imagenAnalizada.imperfecciones.map(i => i.y))} - ${Math.max(...imagenAnalizada.imperfecciones.map(i => i.y))}</p>
              </div>
            </div>
          </div>
          ...
          <div style="background-color: #fef3c7; padding: 15px; border-radius: 4px; font-family: monospace; font-size: 10px; max-height: 300px; overflow-y: auto;">
            <table style="width: 100%; border-collapse: collapse;">
              ...
              <tbody>
                ${imagenAnalizada.imperfecciones.map((imperfeccion, index) => `
      ...
      ${imagenAnalizada?.color_dominante ? `
        <div style="border: 2px solid #8b5cf6; border-radius: 8px; padding: 20px; margin-bottom: 30px;">
          ...
          <div style="display: flex; align-items: center; gap: 15px;">
            <div style="width: 40px; height: 40px; border-radius: 50%; border: 2px solid #d1d5db; background-color: ${safeColor(imagenAnalizada.color_dominante)};"></div>
            <span style="font-size: 16px;"><strong>Color Dominante:</strong> ${imagenAnalizada.color_dominante || 'No disponible'}</span>
          </div>
        </div>
      ` : ''}
    `;
